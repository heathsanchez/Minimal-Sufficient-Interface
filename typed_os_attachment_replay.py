"""End-to-end causal replay for typed attachment safety (authenticated transition).

A locally verified theorem is admitted as knowledge but must not change global
search until attachment is certified by the frozen authority.  We intervene by
attaching a capability whose claim the authority verifies and whose provenance
references the residual, verify the route changes, then ablate the verified
capability and require the unsafe promotion block to return.

COMPATIBILITY CHANGE: ``attach`` and ``promote_global`` no longer accept a
Boolean ``attachment_certificate`` on the envelope as proof.  They require a
capability verified by the frozen verifier (``__init__(verifier=...)``) whose
provenance references the residual witness.
"""
...
def authority(state, capability):
    # The frozen external authority: only the saturated-phase theorem passes.
    return capability.id == 'cap:symbolic-phase'
...
def mk_capability():
    # provenance references the residual-envelope witness installed below
    return Capability(
        'cap:symbolic-phase', 'saturated-four-row-subsystem',
        ('symbolic-phase',), 1.0, ('residual-envelope:0',),
    )


def blocked_phase():
    os = TypedDevelopmentalOperatingSystem(verifier=authority)
    s = mk_state()
    os.install_residual_envelope(s, envelope(False))
    os.wake_actions(s)
    os.invariant_guard(s)
    assert s.residual_type == 'ATTACHMENT'
    assert [a.id for a in s.action_queue] == ['act:attach']
    assert not os.can_promote_globally()
    try:
        os.promote_global(s, 'cap:symbolic-phase')
    except RuntimeError:
        pass
    else:
...
if __name__ == '__main__':
    ...
    # Verified bridge intervention: attach a capability the authority verifies.
    os.attach(s, envelope(True), mk_capability())
    os.invariant_guard(s)
    promoted = os.promote_global(s, 'cap:symbolic-phase')
    after = {'type': s.residual_type, 'promotable': os.can_promote_globally(),
             'promoted': promoted}
    assert after['promotable'] and after['type'] != 'ATTACHMENT'

    # Exact ablation: rebuild identical state but omit the verified capability.
    ...
    import json
    print(json.dumps({
        'before_attachment': before, 'with_attachment': after,
        'certificate_ablated': ablated,
        'causal_consequence': 'verified attachment alone changes global promotion safety and next action',
