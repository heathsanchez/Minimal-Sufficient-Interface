...
def canonical_worlds():
    # Quotient coordinate presentations by abstract finite-abelian-group type.
    reps = {}
...
def response(inv, lens, q):
    """Binary-only lens predicates for the same hidden rank-2 object.

    The learner receives only True/False.  It is never told d1, d2, a bridge,
    a composition result, a point coordinate, or another lens's queries.
    """
    (a, b), (c, d) = q
    if lens == "arithmetic":
        # Equality of two residue classes in an unknown Z/d1 x Z/d2.
        return congruent(inv, (a, b), (c, d))
    if lens == "algebraic":
        # Equality of Laurent monomials on hidden mu_d1 x mu_d2.
        return congruent(inv, (a, b), (c, d))
    if lens == "topological":
        # Two anonymous lattice walks on a hidden discrete torus end together.
        return congruent(inv, (a, b), (c, d))
    if lens == "categorical":
        # Equality of commuting action words A^a B^b and A^c B^d.
        return congruent(inv, (a, b), (c, d))
...
def base_queries():
    ...
    # Use identity comparisons plus genuinely mixed comparisons; this makes both
    # independent periods necessary while keeping the query space bounded.
    qs = []
    z = (0, 0)
    for p in pts:
        if p != z:
...
def quotient_first_generator(inv, r):
    """Intervene by imposing r*A = 0, then return quotient Smith invariants."""
    d1, d2 = inv
    # In C_d1, quotient by subgroup generated by r has order gcd(d1,r).
    a = math.gcd(d1, r)
    b = d2
    return smith(a, b)
...
class FinalLeapCrossLensRank2World(unittest.TestCase):
    def test_rank2_cross_lens_genesis_intervention_and_ablation(self):
        ...
        # Ensure the family contains same-cardinality non-isomorphic groups.
        by_order = {}
        for inv in universe:
        ...
        for true_inv in universe:
            ...
            for lens in LENSES:
                ...
                # The order-only ablation is deliberately deprived of the second
                # structural invariant.  Count tasks it cannot determine, and use
                # a deterministic first-candidate guess to expose actual errors.
                same_order = [inv for inv in universe if inv[0]*inv[1] == true_inv[0]*true_inv[1]]
                if len(same_order) > 1:
                    guess = same_order[0]
                    for q in query_language(lens):
            ...
            # Algebraic-geometric bridge: over C, mu_d1 x mu_d2 has exactly
            # d1*d2 reduced points; its coordinatewise multiplication has the
            # recovered finite-abelian-group law.  We check the finite structural
            # content here without numerically approximating complex roots.
            d1, d2 = true_inv
            algebraic_points = d1 * d2
            if algebraic_points != len(canonical_table(true_inv)) ** 0.5:
                algebraic_bridge_failures += 1

            # Recursive reuse under structural intervention: after learning the
            # object, impose r*A=0 and predict the entire post-intervention binary
            # task language without any new identification queries.
            for r in (2, 3, 4, 5):
