...
INPUTS = (0, 15, 12, 10)  # false, true, x, y
MAX_GATES = 4
...
def exact_search(state, target):
    ...
    ops = operations(state)
    queue = deque([(Program((), i), INPUTS) for i in range(len(INPUTS))])
    for p, wires in queue:
        if wires[p.output] == target:
            return p
    queue = deque([((), INPUTS)])
...
def synthesize(oracle):
    ...
    if arity == 0:
        return None
    proposal = (arity, observed)
    return oracle.certify(proposal)
