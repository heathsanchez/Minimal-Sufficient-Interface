...
def learnedTable : Table := @@LEARNED@@
def acquisitionTable : Table := @@ACQUISITION_TABLE@@
def heldoutTable : Table := @@HELDOUT_TABLE@@
...
def acquired : Program := @@ACQUIRED@@
def transferred : Program := @@TRANSFERRED@@
