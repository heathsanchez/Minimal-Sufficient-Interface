"""ARC3 Global Flash Closure V4.

A falsification experiment for structural, non-linear cross-game development.

The V3 result separated global scheduling from direct cross-game preference
transfer: the global market helped, while safe preference transfer collapsed to
the market. V4 therefore transfers *search-space structure*, not action value.

Arms under the same frozen MG-ARC5 and the same public three-game step budget:

1. sequential_local
   Independent contiguous development.
2. market_local
   One asynchronous global acquisition market; all evidence remains game-local.
3. flash_structural
   The same market plus portable NOOP-family obstruction constructors.

A portable constructor is deliberately conservative:
- it is proposed only when the same typed action schema is observed as NOOP in
  both other games;
- it never installs an exact destination edge;
- at a destination state, two exact local NOOP witnesses of that same schema are
  required before untested sibling actions are removed from the acquisition
  frontier;
- those siblings are recorded as search eliminated, not as proved equivalent.

Thus the transferred object is not "action X is good/bad". It is a hypothesis
about which local experiments are redundant. Destination evidence remains the
sole authority for exact edges, fatality, equivalence and protected progress.

The experiment asks whether a verified obstruction learned anywhere can compile
into less future acquisition elsewhere without damaging protected outcomes.
"""
        ...
        *,
        ...
        if shared and unknown:
            ...
            for schema, keys in by_schema.items():
                constructor = model.portable_noop_constructor(
                    self.game_id,
                    schema,
                )
        ...
        if unknown:
            ...
            for key in unknown:
                ...
                # Structural Flash does not transfer action desirability. The
                # proposal score is destination-local in every arm.
...
def run_market(
    ...
    *,
    ...
    arm = "flash_structural" if shared else "market_local"
    worlds = fresh_worlds(module, manifest, arm)
...
def main() -> None:
    ...
    arms["flash_structural"] = run_market(
        module,
        manifest,
        shared=True,
    )
    print(
        ...
        + json.dumps(
            {
                k: v
                for k, v in arms["flash_structural"].items()
                if k != "trace_head"
            },
            sort_keys=True,
        ),
    ...
    report = {
        "schema": "arc3-global-flash-closure-v4",
        "interpretation": (
            "three-game public developmental falsification: global scheduling is held "
            "against a structural Flash arm that transfers only NOOP-family "
            "obstruction constructors; exact destination evidence remains authoritative"
        ),
        "claim_boundary": (
            "cross-game evidence may propose a redundancy constructor, but two exact "
            "destination-local NOOP witnesses on the exact state/schema are required "
            "before sibling actions leave the acquisition frontier; pruned siblings "
            "are not installed as exact edges or certified equivalences"
        ),
        "games": list(GAMES),
        ...
        "hypothesis_status": {
            "flash_protected_not_worse_than_sequential": all(
                arms["flash_structural"]["games"][game]["max_level"]
                >= arms["sequential_local"]["games"][game]["max_level"]
                for game in GAMES
            ),
            "flash_consequence_yield_gt_sequential": (
                arms["flash_structural"]["consequence_yield_per_step"]
                > arms["sequential_local"]["consequence_yield_per_step"]
            ),
            "flash_cross_game_revaluation_activated": (
                arms["flash_structural"]["metrics"].get(
                    "cross_game_revaluations", 0
                ) > 0
            ),
            "flash_structural_pruning_activated": (
                arms["flash_structural"].get("structural_pruned_actions", 0) > 0
            ),
            ...
            "flash_effective_yield_gt_market": (
                arms["flash_structural"]["effective_consequence_yield_per_step"]
                > arms["market_local"]["effective_consequence_yield_per_step"]
            ),
            "flash_zero_change_le_market": (
                arms["flash_structural"]["zero_change_steps"]
    ...
    audit.write_json(OUT / "global-flash-closure-v4.json", report)
    ...
    print("ARC3_GLOBAL_FLASH_CLOSURE_V4=PASS", flush=True)
