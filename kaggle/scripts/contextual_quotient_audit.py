"""Matched MG-ARC8 contextual-quotient diagnostic against qualified MG-ARC5.

Public worlds are development diagnostics only. A quotient certificate is
bounded empirical evidence from observed transitions, never universal
...
OUT = ROOT / "kaggle" / "contextual-quotient-results"
...
VARIANTS = ("candidate", "prior", "no_contextual_quotient")
...
def prepare() -> None:
    ...
    print("ARC3_CONTEXTUAL_QUOTIENT_MANIFESTS=PASS", flush=True)
...
def cell(args) -> None:
    ...
    def measured(policy, env, digest, **kwargs):
        ...
        if args.variant == "no_contextual_quotient":
            policy.controller.contextual_quotient_enabled = False
        result = old_session(policy, env, digest, **kwargs)
        controller = policy.controller
        quotient = getattr(controller, "contextual_quotient", None)
        ...
        if effects is not None:
            ...
            qcontexts = {value for value in contexts if value.startswith("cq:")}
        certs = list(quotient.certificates()) if quotient is not None else []
        result.update(
            variant=args.variant,
            controller=type(controller).__name__,
            prior_commit=PRIOR,
            contextual_quotient_enabled=bool(
                getattr(controller, "contextual_quotient_enabled", False)
            ),
            quotient_certificates=certs,
            quotient_active_contexts=int(
                getattr(quotient, "active_contexts", 0) if quotient else 0
            ),
            learned_effect_contexts=len(contexts),
            learned_quotient_contexts=len(qcontexts),
        )
        return result
...
def compact(row):
    keys = (
        ...
        "quotient_active_contexts", "quotient_certificates",
        "learned_effect_contexts", "learned_quotient_contexts",
        "contextual_quotient_enabled", "memory_digest",
    )
...
def batch() -> None:
    ...
    print("CONTEXTUAL_QUOTIENT_COMPARISON=" + json.dumps(report, sort_keys=True), flush=True)
    print("ARC3_CONTEXTUAL_QUOTIENT_QUALIFICATION=PASS", flush=True)
