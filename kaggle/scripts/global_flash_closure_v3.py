...
V3 compiles the V2 obstruction instead of adding more transfer weight:
- cross-game evidence is an ephemeral prior only for locally unseen schemas;
- once a destination has direct evidence, destination evidence has sole authority;
- contradicted/uncorroborated cross-game evidence is recorded as an obstruction
  but receives no positive exploration bonus;
- a 12% per-game acquisition floor prevents a shared prior from starving an
  entire experimental lens while preserving global asynchronous allocation.
...
def main() -> None:
    ...
    report = {
        ...
        "interpretation": (
            ...
            "V3 compiles the V2 market-distortion obstruction with ephemeral priors "
            "and a minimum acquisition-share guard"
        ),
        "claim_boundary": (
            "cross-game information can rank only locally unseen experiments after "
            "public-context typing and corroboration; any destination-local evidence "
            "immediately supersedes the prior, contradictions receive zero desirability "
            "bonus, and exact edges, fatality, equivalence and compiled routes remain "
            "destination-local"
        ),
        "games": list(GAMES),
    ...
    audit.write_json(OUT / "global-flash-closure-v3.json", report)
