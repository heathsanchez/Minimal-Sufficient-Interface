"""Matched MG-ARC7 resource-factor diagnostic against qualified MG-ARC5."""
...
OUT = ROOT / "kaggle" / "resource-factor-results"
...
VARIANTS = ("candidate", "prior", "no_resource_factor")
...
def prepare() -> None:
    ...
    for kind in ("fixture", "public"):
        ...
        write(path, manifest)
    print("ARC3_RESOURCE_FACTOR_MANIFESTS=PASS", flush=True)
...
def cell(args) -> None:
    ...
    def measured(policy, env, digest, **kwargs):
        ...
        if args.variant == "no_resource_factor":
            policy.controller.resource_factor_enabled = False
        result = old_session(policy, env, digest, **kwargs)
        controller = policy.controller
        factor = getattr(controller, "resource_factor", None)
        effects = getattr(controller, "effects", None)
        contexts = set()
        if effects is not None:
            contexts.update(source for source, _ in effects.edges)
            contexts.update(effects.catalogs)
            for row in effects.edges.values():
                contexts.update(str(target) for target in row.get("outcomes", {}))
        world_contexts = {ctx for ctx in contexts if str(ctx).startswith("w:")}
        result.update(
            variant=args.variant,
            controller=type(controller).__name__,
            prior_commit=PRIOR,
            resource_factor_enabled=bool(getattr(controller, "resource_factor_enabled", False)),
            resource_active_contexts=int(getattr(factor, "active_contexts", 0) if factor else 0),
            resource_masked_pixels=int(getattr(factor, "masked_pixel_count", 0) if factor else 0),
            resource_transition_records=len(factor.resource_transitions()) if factor else 0,
...
def compact(row):
    keys = (
        ...
        "resource_active_contexts", "resource_masked_pixels",
        "resource_transition_records", "learned_effect_contexts",
        "learned_world_contexts", "resource_factor_enabled", "memory_digest",
    )
...
def batch() -> None:
    ...
    print("RESOURCE_FACTOR_COMPARISON=" + json.dumps(report, sort_keys=True), flush=True)
    print("ARC3_RESOURCE_FACTOR_QUALIFICATION=PASS", flush=True)
