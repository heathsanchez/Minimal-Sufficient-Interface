"""Matched MG-ARC8 certified contextual-quotient diagnostic.

Public worlds are development diagnostics, not sealed holdouts or competition
scores. Exact MG-ARC5 memory/requalification remains the protected baseline.
"""
...
OUT = ROOT / "kaggle" / "context-quotient-results"
...
def prepare() -> None:
    ...
    for kind in ("fixture", "public"):
        ...
        write(path, manifest)
    print("ARC3_CONTEXT_QUOTIENT_MANIFESTS=PASS", flush=True)
...
def cell(args) -> None:
    ...
    def measured(policy, env, digest, **kwargs):
        ...
        if args.variant in ("prior", "no_quotient"):
            if hasattr(policy.controller, "contextual_quotient_enabled"):
        ...
        if effects is not None:
            ...
            qcontexts = {ctx for ctx in contexts if str(ctx).startswith("cq:")}
        records = list(quotient.active_records()) if quotient is not None else []
        result.update(
...
def compact(row):
    keys = (
        ...
        "quotient_active_contexts", "quotient_records",
...
def batch() -> None:
    ...
    print("CONTEXT_QUOTIENT_COMPARISON=" + json.dumps(report, sort_keys=True), flush=True)
    print("ARC3_CONTEXT_QUOTIENT_QUALIFICATION=PASS", flush=True)
