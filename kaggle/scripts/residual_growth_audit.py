"""Matched MG-ARC6 residual-growth diagnostic against qualified MG-ARC5.

Public worlds are development diagnostics, not sealed holdouts or competition
scores. The stable quotient is a learned consequence-state abstraction only;
MG-ARC5 certified substitution/refutation contexts remain exact.
"""
...
OUT = ROOT / "kaggle" / "residual-growth-results"
...
def prepare() -> None:
    ...
    for kind in ("fixture", "public"):
        ...
        write(path, manifest)
    print("ARC3_RESIDUAL_GROWTH_MANIFESTS=PASS", flush=True)
...
def cell(args) -> None:
    ...
    def measured(policy, env, digest, **kwargs):
        ...
        if args.variant == "no_quotient":
            policy.controller.stable_state_enabled = False
        result = old_session(policy, env, digest, **kwargs)
        controller = policy.controller
        quotient = getattr(controller, "state_quotient", None)
        effects = getattr(controller, "effects", None)
        effect_contexts = set()
        q_contexts = set()
        if effects is not None:
            for (source, _action), row in effects.edges.items():
                effect_contexts.add(source)
                effect_contexts.update(str(target) for target in row.get("outcomes", {}))
            effect_contexts.update(effects.catalogs)
            q_contexts = {ctx for ctx in effect_contexts if str(ctx).startswith("q:")}
        result.update(
            variant=args.variant,
            controller=type(controller).__name__,
            prior_commit=PRIOR,
            stable_state_enabled=bool(getattr(controller, "stable_state_enabled", False)),
            quotient_active_contexts=int(getattr(quotient, "active_contexts", 0) if quotient else 0),
            quotient_masked_pixels=int(getattr(quotient, "masked_pixel_count", 0) if quotient else 0),
            learned_effect_contexts=len(effect_contexts),
            learned_quotient_contexts=len(q_contexts),
        )
        return result
...
def compact(row):
    keys = (
        ...
        "quotient_active_contexts", "quotient_masked_pixels",
        "learned_effect_contexts", "learned_quotient_contexts",
        "stable_state_enabled", "memory_digest",
    )
...
def batch() -> None:
    ...
    for kind in ("fixture", "public"):
        ...
        for game in manifest["games"]:
            ...
            # Until its 64-transition residual gate activates, MG-ARC6 must be
            # behaviorally identical to its no-quotient ablation.
            records.append(
                {
                    "kind": kind,
                    "game_id": gid,
    ...
    print("RESIDUAL_GROWTH_COMPARISON=" + json.dumps(report, sort_keys=True), flush=True)
    print("ARC3_RESIDUAL_GROWTH_QUALIFICATION=PASS", flush=True)
