"""Matched MG-ARC7 motion-model diagnostic against qualified MG-ARC5.

Public worlds remain development diagnostics. The motion model learns only exact
one-to-one connected-component translations and never receives game IDs, goals,
source code, or hidden environment state.
"""
...
OUT = ROOT / "kaggle" / "motion-model-results"
...
VARIANTS = ("candidate", "prior", "no_motion")
...
def prepare() -> None:
    ...
    for kind in ("fixture", "public"):
        ...
        write(path, manifest)
    print("ARC3_MOTION_MODEL_MANIFESTS=PASS", flush=True)
...
def cell(args) -> None:
    ...
    def measured(policy, env, digest, **kwargs):
        ...
        if args.variant == "no_motion":
            policy.controller.motions.recommend = lambda *a, **k: None
        result = old_session(policy, env, digest, **kwargs)
        motions = getattr(policy.controller, "motions", None)
        vector_rows = len(getattr(motions, "_vectors", {})) if motions is not None else 0
        position_classes = len(getattr(motions, "_positions", {})) if motions is not None else 0
        result.update(
            variant=args.variant,
            controller=type(policy.controller).__name__,
            prior_commit=PRIOR,
            reliable_motion_controls=int(
                getattr(motions, "reliable_control_count", 0) if motions is not None else 0
            ),
            motion_vector_rows=vector_rows,
            motion_component_classes=position_classes,
        )
        return result
...
def compact(row):
    keys = (
        ...
        "reliable_motion_controls", "motion_vector_rows",
        "motion_component_classes", "memory_digest",
    )
...
def batch() -> None:
    ...
    print("MOTION_MODEL_COMPARISON=" + json.dumps(report, sort_keys=True), flush=True)
    print("ARC3_MOTION_MODEL_QUALIFICATION=PASS", flush=True)
