"""Matched MG-ARC8 border-scalar diagnostic against qualified MG-ARC5."""
...
OUT = ROOT / "kaggle" / "border-scalar-results"
...
def prepare():
    ...
    print("ARC3_BORDER_SCALAR_MANIFESTS=PASS", flush=True)
...
def cell(args):
    ...
    path = OUT / f"{args.kind}-{args.variant}.json"
    audit.write_json(path, m)
    ...
    def measured(policy, env, digest, **kwargs):
        ...
        if args.variant == "no_factor":
            policy.controller.border_factor_enabled = False
        ...
        world_effects = getattr(c, "world_effects", None)
        world_contexts = set()
        if world_effects is not None:
            for (source, _a), row in world_effects.edges.items():
                world_contexts.add(source)
                world_contexts.update(row.get("outcomes", {}))
        result.update(
            variant=args.variant,
            factor_enabled=bool(getattr(c, "border_factor_enabled", False)),
            factor_candidates=candidates,
            factor_active_contexts=len(candidates),
            world_effect_observations=int(getattr(world_effects, "total_observations", 0) if world_effects else 0),
            world_effect_contexts=len(world_contexts),
            exact_effect_observations=int(getattr(getattr(c, "effects", None), "total_observations", 0)),
            prior_commit=PRIOR,
        )
        return result
...
def compact(r):
    keys = (
        ...
        "factor_enabled","factor_candidates","factor_active_contexts",
        "world_effect_observations","world_effect_contexts","exact_effect_observations",
    )
...
def run():
    ...
    for kind in ("fixture","public"):
        ...
        for game in m["games"]:
            ...
            for variant in VARIANTS:
                ...
                paired[variant]=row
                print("BORDER_SCALAR_CELL="+json.dumps(
    ...
    print("BORDER_SCALAR_COMPARISON="+json.dumps(report,sort_keys=True),flush=True)
    print("ARC3_BORDER_SCALAR_QUALIFICATION=PASS",flush=True)
