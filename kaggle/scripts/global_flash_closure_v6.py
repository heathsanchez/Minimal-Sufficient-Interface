"""ARC3 Global Flash Closure V6.

V4 falsified the first structural constructor: no portable NOOP family survived
across the public three-game bundle. Crucially, however, every multi-game coarse
schema was obstructed: the same representation mixed different consequences.

V6 turns that failure into the transferable object.

Instead of transferring action value or claiming cross-game equivalence,
cross-game contradiction promotes a REPRESENTATION_REFINEMENT capability. The
destination then scores future experiments in a finer schema while all exact
edges, outcomes and protected progress remain strictly destination-local.

Arms:
1. sequential_local -- independent contiguous development.
2. market_local -- asynchronous global budget allocation with local evidence.
3. flash_refinement -- the same global market plus obstruction-triggered
   representation refinement and a persistent Flash capability ledger.

The test is whether failures in one lens can improve the representation used by
other live lenses, reducing acquisition waste or improving protected progress
without transferring answers.
"""
        ...
        if unknown:
            ...
            for key in unknown:
                ...
                if shared and structural_model is not None:
                    refinement = structural_model.refinement_obstruction(schema)
                    if refinement is not None:
                        effective_schema = refined_schema_for(key, obs)
                        self.refined_schema_ids.add(effective_schema)

                # Action desirability remains destination-local. Flash changes
                # the representation in which local evidence is accumulated.
                ...
                if refinement is not None:
                    ...
                    meta["refinement_capability"] = refinement
                if shared and structural_model is not None:
                    constructor = structural_model.portable_noop_constructor(
                        self.game_id,
                        schema,
...
def write_refinement_ledger(model: ProposalModel) -> dict[str, Any]:
    ledger = FlashLedger()
    promoted = 0
    for schema in sorted(model.counts, key=repr):
        ...
        for game in obstruction["source_games"]:
            ...
            ref = EvidenceRef(
                ...
                artifact_or_run="arc3-global-flash-closure-v6",
...
def execute_and_close(
    ...
    *,
    ...
    if candidate.kind == "probe" and candidate.schema is not None:
        ...
        effective_schema = candidate.metadata.get(
            "effective_schema",
            candidate.schema,
        )
        local_model.add(world.game_id, tuple(effective_schema), signature)
...
def run_market(
    ...
    *,
    ...
    arm = "flash_refinement" if shared else "market_local"
    worlds = fresh_worlds(module, manifest, arm)
    ...
    try:
        ...
        if shared_model is not None:
            ...
            report["flash_ledger"] = write_refinement_ledger(shared_model)
        return report
    finally:
        close_worlds(worlds)
...
def main() -> None:
    ...
    arms["flash_refinement"] = run_market(
        module,
        manifest,
        shared=True,
    )
    print(
        ...
        + json.dumps(
            {
                k: v
                for k, v in arms["flash_refinement"].items()
                if k != "trace_head"
            },
            sort_keys=True,
        ),
    ...
    report = {
        "schema": "arc3-global-flash-closure-v6",
        "interpretation": (
            "three-game public developmental falsification: global scheduling is held "
            "against a Flash arm where cross-game contradiction promotes a finer "
            "destination representation rather than transferring action value"
        ),
        "claim_boundary": (
            "cross-game contradiction may activate a deterministic public-geometry "
            "refinement, but local proposal scores, exact edges, fatality, equivalence "
            "and protected outcomes remain destination-local; no foreign outcome is "
            "installed as a destination fact"
        ),
        "games": list(GAMES),
        ...
        "hypothesis_status": {
            "flash_protected_not_worse_than_sequential": all(
                arms["flash_refinement"]["games"][game]["max_level"]
                >= arms["sequential_local"]["games"][game]["max_level"]
                for game in GAMES
            ),
            "flash_consequence_yield_gt_sequential": (
                arms["flash_refinement"]["consequence_yield_per_step"]
                > arms["sequential_local"]["consequence_yield_per_step"]
            ),
            "flash_cross_game_revaluation_activated": (
                arms["flash_refinement"]["metrics"].get(
                    "cross_game_revaluations", 0
                ) > 0
            ),
            "flash_representation_refinement_activated": (
                arms["flash_refinement"].get(
                    "representation_refinement_uses", 0
                ) > 0
            ),
            ...
            "flash_effective_yield_gt_market": (
                arms["flash_refinement"]["effective_consequence_yield_per_step"]
                > arms["market_local"]["effective_consequence_yield_per_step"]
            ),
            "flash_zero_change_le_market": (
                arms["flash_refinement"]["zero_change_steps"]
    ...
    audit.write_json(OUT / "global-flash-closure-v6.json", report)
    ...
    print("ARC3_GLOBAL_FLASH_CLOSURE_V6=PASS", flush=True)
