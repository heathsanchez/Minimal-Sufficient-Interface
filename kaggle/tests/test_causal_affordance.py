...
class CausalAffordanceContracts(unittest.TestCase):
    def test_parameterized_action_prefers_spatially_aligned_effect(self):
        ...
        near = tuple(tuple(3 if (x, y) == (1, 1) else v for x, v in enumerate(row)) for y, row in enumerate(before))
        far = tuple(tuple(3 if (x, y) == (4, 4) else v for x, v in enumerate(row)) for y, row in enumerate(before))
    ...
    def test_repeated_same_effect_across_coordinates_collapses_parameter_equivalence(self):
        ...
        self.assertTrue(mem.parameter_equivalent('shape-x', (6, 1, 1), (6, 9, 9)))

    def test_controllable_descriptor_outranks_novel_but_unaligned_change(self):
        mem = AffordanceMemory(limit=64)
        for i in range(3):
            mem.record(f'near-{i}', (6, 4, 4), 'near', (1, 1, 1, 0, 0, 1), directness=1.0)
        for i in range(3):
            mem.record(f'far-{i}', (6, 20 + i, 20), 'far', (20, 8, 8, 3, 3, 5), directness=0.0)
    ...
    def test_primitive_reversible_effect_is_recognized_as_controllable(self):
        mem = AffordanceMemory(limit=64)
        mem.record('a', (1, None, None), 'primitive:1', (4, 2, 1, 1, 0, 2), directness=1.0, target='b')
        mem.record('b', (2, None, None), 'primitive:2', (4, 2, 1, -1, 0, 2), directness=1.0, target='a')
