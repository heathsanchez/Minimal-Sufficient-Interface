...
_VALID_MODES = {"sham", "consequences", "full"}


def _mode() -> str:
    value = os.environ.get("DUCKTAPE_MODE", "").strip().lower()
    if value not in _VALID_MODES:
        raise ValueError(
            "DUCKTAPE_MODE must be one of sham, consequences, full"
        )
    return value
...
class DuckTapeToolAgent(ToolAgent):
    """Matched DuckTape causal-ablation agent.

    All three modes expose the same DuckTape prompt shell and run the same
    public Duck harness/model/tool surface.

    sham:
        shell only; no earned state persists.
    consequences:
        persist exact observed consequences, including conservative exact-state
        GAME_OVER refusal; do not compile or expose progress programs.
    full:
        consequences plus level-progress programs compiled as capabilities.

    The authoritative state in enabled modes is still only one append-only
    immutable tuple of canonical events.
    """

    def __init__(self, *args: Any, **kwargs: Any) -> None:
        super().__init__(*args, **kwargs)
        self._ducktape_mode = _mode()
        self._ducktape_log: tuple[tuple[str, tuple[Any, ...]], ...] = ()
        self._ducktape_program: list[tuple[Any, ...]] = []
        self._ducktape_blocked = 0

    @property
    def _dt_keeps_consequences(self) -> bool:
        return self._ducktape_mode in {"consequences", "full"}

    @property
    def _dt_keeps_capabilities(self) -> bool:
        return self._ducktape_mode == "full"
    ...
    def _dt_exact_facts(self, state_sig: str) -> dict[tuple[Any, ...], str]:
        if not self._dt_keeps_consequences:
            return {}
    ...
    def _dt_progress_programs(self) -> list[tuple[tuple[Any, ...], ...]]:
        if not self._dt_keeps_capabilities:
            return []
    ...
    def _run_python_tool(self, state_path: Path, arguments: dict[str, Any]):
        ...
        def wrapped_callback(payload: dict[str, Any]) -> dict[str, Any]:
            ...
            if self._dt_keeps_consequences and len(action_keys) == 1:
            ...
            if (
                self._dt_keeps_consequences
                and len(action_keys) == 1
            ...
            if self._dt_keeps_capabilities:
                if action_keys:
...
def patch(root: Path) -> None:
    ...
    print("DUCKTAPE_MODES_PATCH=PASS")
