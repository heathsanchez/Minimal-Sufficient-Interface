...
class ConsequenceController(MemoryGraphController):
    ...
    @staticmethod
    def _protected_observation(obs: Observation) -> tuple[Any, ...]:
        return (
    ...
    def _consequence_context(
        ...
        if (
            not self.contextual_quotient_enabled
            or not grid
            or not self.contextual_quotient.active(len(grid), len(grid[0]))
        ):
            return obs.evidence_sha256
        return (
            f"cq:{obs.levels_completed}:"
            f"{self.contextual_quotient.project(grid)}"
        )
    ...
    def _record_effect(self, frame: Any, obs: Observation) -> None:
        ...
        if self._pending_effect is not None:
            ...
            if (
                self.contextual_quotient_enabled
                ...
                self.contextual_quotient.observe(
                    ...
                    action,
                    grid,
                    self._protected_observation(previous),
                    self._protected_observation(obs),
                )
            source_context = (
                self._consequence_context(previous, before)
