...
OUT=Path(os.environ.get("OUTDIR","evidence/arc3-public-all-blue-to-gray-g2")).resolve()
...
def env():
    l=logging.getLogger("all-blue-gray");l.setLevel(logging.WARNING)
...
def run_once():
    e=env();enter2(e)
    trace=[];actions=0;tried=set()
    initial={"blue3":len(blue3(e.observation_space)),"gray3":len(gray3(e.observation_space))}
    while actions<MAX_ACTIONS:
      if int(e.observation_space.levels_completed)>1 or e.observation_space.state in (GameState.WIN,GameState.GAME_OVER):break
      bs=blue3(e.observation_space)
      if not bs:break
      # Prefer untouched blue components in deterministic spatial order.
      candidates=[]
      for comp in bs:
        cell=comp["cells"][len(comp["cells"])//2]
        if cell not in tried:candidates.append(cell)
      if not candidates:
        break
      rc=sorted(candidates)[0]
      before=len(bs);before_gray=len(gray3(e.observation_space))
      z=click(e,rc);actions+=1;tried.add(rc)
      after=len(blue3(z));after_gray=len(gray3(z))
      trace.append({"action":actions,"rc":list(rc),"blue_before":before,"blue_after":after,
                    "gray_before":before_gray,"gray_after":after_gray,
                    "level":int(z.levels_completed),"state":str(z.state)})
      if z.state==GameState.GAME_OVER:break
    ...
    if int(e.observation_space.levels_completed)==1 and e.observation_space.state==GameState.NOT_FINISHED and actions<MAX_ACTIONS:
      z=click(e,TERMINAL);actions+=1
      trace.append({"action":actions,"rc":list(TERMINAL),"terminal":True,
                    "level":int(z.levels_completed),"state":str(z.state)})
    return {
      "initial":initial,"preterminal":preterminal,"actions":actions,
...
def main():
    ...
    out={
      "hypothesis":"source invariant: recolor every blue size-3 segment to gray, then terminal",
      "terminal":list(TERMINAL),"trials":trials,
      "status":"PROMOTED" if ok else "RESIDUAL",
      "model_calls":0,"source_inspection":False,
      "claim_boundary":"exact public tn36 G2 dynamic all-blue-size3 recolor invariant"
    }
    (OUT/"result.json").write_text(json.dumps(out,indent=2))
    print(json.dumps(out,indent=2))
    print("ARC3_PUBLIC_ALL_BLUE_TO_GRAY_G2="+out["status"])
