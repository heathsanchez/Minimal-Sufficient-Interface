...
OUT = Path(os.environ.get(
    "OUTDIR", "evidence/arc3-public-g4-scale-action-ordering"
)).resolve()
OUT.mkdir(parents=True, exist_ok=True)

PROBES = {"S": (15, 58), "D": (25, 58), "L": (35, 58)}
PATHS = ("LSDDDD", "DSLDDD", "DSDLDD", "LDSDDD", "DLSDDD")
...
def main():
    ...
    result = {
        ...
        "hypothesis": (
            "the scale-normalized mover may translate at most one cell above the wall "
            "before shrinking; test the five collision-free orderings not covered by "
            "the scale-first gate"
        ),
        "paths": list(PATHS),
        "variants": variants,
        "selected": selected,
        ...
        "claim_boundary": (
            "exact public tn36 G4; completes the eight collision-free permutations of "
            "one shrink, four down, and one left action without testing arbitrary matrices"
        ),
    }
    (OUT / "result.json").write_text(json.dumps(result, indent=2, sort_keys=True) + "\n")
    print(json.dumps(result, sort_keys=True), flush=True)
    print(f"ARC3_PUBLIC_G4_SCALE_ACTION_ORDERING={result['status']}", flush=True)
    os._exit(0)
