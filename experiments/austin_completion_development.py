...
This installs a proof-carrying equality shortcut, not a terminating or confluent
rewrite system. A bounded normal-form mismatch is not global unjoinability.
"""
    ...
    for i, outer0 in enumerate(rules):
        for j, inner0 in enumerate(rules):
            ...
            for path, site in walk(ol):
                ...
                nl, tl, ok1 = trace(left, rules)
                nr, tr, ok2 = trace(right, rules)
                if ok1 and ok2 and nl != nr:
                    return dict(rules=rules, pair=(i, j, path), substitution=e,
                                peak=peak, left=left, right=right, left_nf=nl,
                                right_nf=nr, left_trace=tl, right_trace=tr)
    raise ValueError("No bounded residual in current grammar")

def promotion_source(seed):
    path, e = attachment(seed)
    original = instantiate(seed[0], e)
    promoted = replace_at(original, path, V("q"))
    return "\n".join([
        "theorem hProm {G : Type} (op : G → G → G)",
        f"    (hLaw : {law(seed)})",
        "    (A p q w : G) (hBranch : op A p = q) :",
        f"    {term(promoted)} = {term(instantiate(seed[1], e))} := by",
        f"  have hctx : {term(promoted)} = {term(original)} :=",
        f"    congrArg (fun hole => {term(context(original, path))}) hBranch.symm",
        f"  exact hctx.trans (hLaw {' '.join(term(e[v]) for v in 'xyz')})", ""])
...
def step_proof(seed, index, path, e, prefix, source):
    """Check the exact instantiated step before emitting its congruence proof."""
    lhs, rhs = (seed, promote(seed))[index]
    vals = {v: instantiate(V(prefix + v), e) for v in vars(lhs) | vars(rhs)}
    assert at(source, path) == instantiate(lhs, vals), "Proof source does not match rewrite site"
    target = replace_at(source, path, instantiate(rhs, vals))
    proof = f"(congrArg (fun hole => {term(context(source, path))}) {rule_proof(seed, index, e, prefix)})"
    return target, proof
...
def theorem_source(name, seed):
    ...
    def chain(first, tail, index, path, env, prefix):
        current, proof = step_proof(seed, index, path, env, prefix, peak)
        assert current == first, "Critical-pair reduct mismatch"
        for src, nxt, k, p, e in tail:
            assert src == current, "Broken proof chain"
            current, next_proof = step_proof(seed, k, p, e, f"N{k}_", src)
            assert current == nxt, "Normalisation trace mismatch"
            proof = f"(Eq.trans {proof} {next_proof})"
        return current, proof

    left, left_proof = chain(r["left"], r["left_trace"], i, (), r["substitution"], f"O{i}_")
    right, right_proof = chain(r["right"], r["right_trace"], j, path, r["substitution"], f"I{j}_")
    assert left == nl and right == nr
    lines = ["import Std", "", "namespace AustinCompletion" + name, "", promotion_source(seed),
             f"theorem completion{name} {{G : Type}} (op : G → G → G)",
             f"    (hLaw : {law(seed)})",
             f"    ({' '.join(sorted(vars(peak)))} : G) :",
             f"    {term(large)} = {term(small)} := by",
             f"  have hLeft : {term(peak)} = {term(nl)} := by",
             f"    exact {left_proof}",
             f"  have hRight : {term(peak)} = {term(nr)} := by",
             f"    exact {right_proof}",
             "  exact hLeft.symm.trans hRight" if large == nl else "  exact hRight.symm.trans hLeft",
             "", "end AustinCompletion" + name, ""]
...
def verify(state):
    ...
    if state.bound != 3:
        raise ValueError("Bound changed")
    if state.archive and transport_certificate() not in state.parent.archive:
        raise ValueError("Missing transport dependency")
    if len(state.archive) > 1 or any(c != certificate() for c in state.archive):
        raise ValueError("Forged or duplicate capability")
    return True
...
def develop(state, training):
    ...
    r = residual(training)
    cert = certificate()
    after = State(state.parent, state.archive + (cert,))
    verify(after)
...
def lean_source():
    """One import at file start; each generated theorem has its own namespace."""
    parts = ["import Std", ""]
    for name, seed in (("40909", e40909_seed()), ("11116", e11116_seed())):
        standalone = theorem_source(name, seed)[0]
        assert standalone.startswith("import Std\n\n")
        parts.append(standalone[len("import Std\n\n"):])
    return "\n".join(parts)

def qualification():
    from austin_capability_installation import develop as acquire, obligation, law40909
    parent, e = acquire(DevelopmentState(), obligation("E40909", law40909))
    assert e is not None
    before = State(parent)
    after, e = develop(before, e40909_seed())
    assert e is not None and verify(after)
    ...
    return {"training": "E40909", "heldout": "E11116", "baseline_reachable": 0,
            "installed_reachable": 1, "ablation_reachable": 0, "source_bound": after.bound,
            "certificate": e["certificate"].identity,
            "scope": "bounded critical-pair equality; external Lean required"}
