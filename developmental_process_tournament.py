...
CASES = [
    ...
    ('Prove the claimed theorem symbolically from the retained laws.', 'DERIVE'),
...
HELDOUT = [
    ...
    ('Derive the equivalence as lemmas without exhaustive enumeration.', 'DERIVE'),
...
EXPECTED_FROM_TYPE = {
    'REPRESENTATION':'REFRAME','OBSERVABLE':'PROBE','VERIFIER':'VERIFY','DERIVATION':'DERIVE','INFRA':'INFRA',
...
def score(policy, cases):
    ...
    for residual,expected in cases:
        got=policy(residual); ok=got==expected; correct+=ok
        rows.append({'residual':residual,'expected':expected,'got':got,'ok':ok})
    return correct, rows

def main():
    os=DevelopmentalOperatingSystem()
    b_dev,brows=score(baseline_policy,CASES); r_dev,rrows=score(lambda x:routed_policy(os,x),CASES)
    b_hold,bhrows=score(baseline_policy,HELDOUT); r_hold,rhrows=score(lambda x:routed_policy(os,x),HELDOUT)
    assert r_dev > b_dev, (b_dev,r_dev,rrows)
    assert r_hold > b_hold, (b_hold,r_hold,rhrows)
    assert r_dev == len(CASES), rrows
    assert r_hold == len(HELDOUT), rhrows
    import json
