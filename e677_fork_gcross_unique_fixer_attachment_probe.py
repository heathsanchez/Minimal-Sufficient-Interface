...
from e677_fork_gcross_ground_e677_probe import C, BAD, names, e677, structural_assertions


def no_bad_fixer_assertions():
    ...
    return [C[r] * 0 == C[r] * 0 for r in []]  # placeholder eliminated below
...
def add_named_no_fixers(s: Solver):
    for r_name in names:
        row = C[r_name]
        for u in BAD:
            s.add(__import__('e677_fork_gcross_ground_e677_probe').mul(row, u) != u)
...
def main():
    ...
    attached_res,attached_solver=solve(add_e677=True,add_no_fixers=True)
